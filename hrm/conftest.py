"""Pytest configuration for hrm — generated by phoenix init."""
...
def page(context, intelligent_runtime, request) -> Page:
    ...
    if intelligent_runtime:
        try:
            ...
            test_name = request.node.name if hasattr(request, 'node') else "hrm_test"
            execution_id = getattr(intelligent_runtime, 'execution_id', '')
            project_name = getattr(intelligent_runtime, 'project_name', 'hrm')
...
def pytest_sessionfinish(session, exitstatus):
    ...
    # when using `phoenix run`. For direct pytest execution, the old
    # runtime wrapper summary is still used if available.
...
@pytest.fixture(scope="session")
def phoenix_intelligent_runtime():
    """Phoenix intelligent runtime for DOM reuse and self-learning execution."""
    if INTELLIGENT_RUNTIME_AVAILABLE:
        import time
        runtime = IntelligentRuntime(
            base_dir="PhoenixRuntime",
            project_name="hrm",
            enable_cache=True,
        ...
        execution_id = runtime.start_execution("pytest_session")
        yield runtime
        # End execution tracking
        runtime.end_execution(status="passed" if exitstatus == 0 else "failed")
    return None


# ---------------------------------------------------------------------------
# Phoenix Runtime Integration Fixture (Legacy)
# ---------------------------------------------------------------------------

@pytest.fixture(scope="session")
def phoenix_runtime():
    """Phoenix runtime integration for enterprise subsystem visibility (legacy)."""
    if PHOENIX_RUNTIME_AVAILABLE:
...
def intelligent_runtime():
    ...
    # Initialize execution state for fallback
    execution_id = f"fallback_{time.time()}"
    intelligent_runtime.start_execution("fallback_test")
    print(f"[INTELLIGENT RUNTIME] Fallback execution started: {execution_id}")
    return intelligent_runtime
