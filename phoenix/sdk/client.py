...
class PhoenixClient:
    ...
    def __init__(self, config: Optional[PhoenixConfig] = None, config_path: Optional[str] = None):
        ...
        self._agent_registry = AgentRegistry(self._knowledge_base, self._cache)
        
        self._manual_generator = ManualTestGenerator(
            self._agent_registry,
            output_dir="./manual_tests"
        )
    ...
    def generate_tests(
        ...
        **kwargs
        ...
        if test_type in ["manual", "both"]:
            # Remove risk_level from kwargs if present to avoid duplicate
            manual_kwargs = {k: v for k, v in kwargs.items() if k != "risk_level"}
        ...
        if test_type in ["automation", "both"]:
            ...
            auto_kwargs = {k: v for k, v in kwargs.items() if k != "test_type"}
            automation_test_data = self._automation_generator.generate(
                user_story=user_story,
                application_url=application_url,
                acceptance_criteria=acceptance_criteria,
                test_category=auto_kwargs.get("test_category", "ui"),
                **auto_kwargs
            )
